...
end-to-end and writes a consolidated report at
``_audits/evaluation_results.md``.
...
8. Synthesise everything into ``_audits/evaluation_results.md``.

CLI::

    python -m scripts.evaluate_id3 --workers 24 \\
        --output-dir data/id3 --report _audits/evaluation_results.md
...
def freeze_splits(
    ...
    Audit finding A (high severity, ``_audits/sweep_results.md``§7A):
...
def write_report(
    ...
    *,
    ...
    lines.append("- **Position-duplication ablation (audit §7C).** Not "
                 "executed in this prompt — would require a separate "
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--report", type=Path, default=Path("_audits/evaluation_results.md"),
