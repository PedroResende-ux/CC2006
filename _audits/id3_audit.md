...
| **Split** | `stratified_split` + `split_dataset` ([ai/dt_pipeline.py:360](ai/dt_pipeline.py:360)) | Class-stratified nested: first 80/20 train+val/test, then 90/10 of train+val → final 72/8/20 of the original. Pure numpy/pandas (no sklearn). | Three-way split — uses validation set for hyper-parameter tuning. **No k-fold cross-validation.** |
